...
import { deleteQuestionnaireById } from 'apiSdk/questionnaires';
...
function ClinicViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ClinicInterface>(
    ...
    () =>
      getClinicById(id, {
        relations: ['user', 'document', 'questionnaire'],
      }),
  );
  ...
  const questionnaireHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('document', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/documents/create?clinic_id=${data?.id}`}>
              ...
              {hasAccess('questionnaire', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/questionnaires/create?clinic_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.questionnaire?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/questionnaires/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.title}</Td>
                            ...
                            <Td>
                              {hasAccess('questionnaire', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/questionnaires/edit/${record.id}`}
                                >
                                  <Button mr={2} as="a" variant="outline" colorScheme="blue" leftIcon={<FiEdit2 />}>
                                    Edit
                                  </Button>
                              ...
                              {hasAccess('questionnaire', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    questionnaireHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
